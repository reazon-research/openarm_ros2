...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_OPENARM_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_OPENARM_CONTROLLER_HPP_
...
#include "openarm_controller/openarm_controller.hpp"
...
using ControllerStateMsg = openarm_controller::OpenarmController::ControllerStateMsg;
using ControllerReferenceMsg = openarm_controller::OpenarmController::ControllerReferenceMsg;
using ControllerModeSrvType = openarm_controller::OpenarmController::ControllerModeSrvType;
...
class TestableOpenarmController : public openarm_controller::OpenarmController
{
  FRIEND_TEST(OpenarmControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(OpenarmControllerTest, activate_success);
  FRIEND_TEST(OpenarmControllerTest, reactivate_success);
  FRIEND_TEST(OpenarmControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(OpenarmControllerTest, test_update_logic_fast);
  FRIEND_TEST(OpenarmControllerTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = openarm_controller::OpenarmController::on_configure(previous_state);
...
class OpenarmControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_openarm_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_openarm_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_openarm_controller")
  {
    ASSERT_EQ(
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_openarm_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableOpenarmController> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_OPENARM_CONTROLLER_HPP_
